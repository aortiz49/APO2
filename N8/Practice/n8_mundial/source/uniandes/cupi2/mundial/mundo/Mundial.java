...
public class Mundial {
     ...
     * Si el archivo indicado no existe, entonces el mundial se crea vacío y su estado se
     * guardará en el archivo indicado.<br>
     ...
     * @throws PersistenciaException Se lanza esta excepción si se encuentran problemas cargando
     ...
     * @throws ElementoExisteException Esta excepción se lanza si ya existe un equipo con el
     *                                 mismo nombre
     */
    public void agregarEquipo(String pais, String director, String imagen)
     ...
     *
     * @param nombreEquipo El nombre del equipo para adicionar el jugador - hay un equipo con ese
     *                     nombre en el mundial
     ...
     * @throws ElementoExisteException Esta excepción se lanza si ya existe otro jugador en el
     *                                 equipo con el mismo nombre
     */
    public void agregarJugadorAEquipo(String nombreEquipo, String nombreJ, int edadJ,
                                      String posicionJ, double alturaJ, double pesoJ,
                                      double salarioJ, String imagenJ)
            throws ElementoExisteException {
     ...
     * @throws IOException Se genera esta excepción si hay problemas salvando el archivo con el
     *                     reporte
     */
    public String calcularValorNomina(String nombreEquipo, String rutaReporte) throws IOException {
        ...
        out.println("Total Nómina:          " + darEquipo(nombreEquipo).calcularValorNomina()
                            + " millones anuales");
        out.close();
    ...
    /**
     * Actualiza la información de los jugadores a partir de la información contenida en un
     * archivo de texto.<br>
     * El archivo debe tener una línea en la cual se encuentra el número de jugadores a los que
     * se les va a modificar la información, <br>
     * y luego debe haber una línea por cada jugador al que se le va a modificar la información.
     * <br>
     * Cada línea tiene el siguiente formato: <nombre país>;<nombre jugador>;<edad>;<posición>;
     * <altura>;<peso>;<salario>;<imágen>
     *
     ...
     *                                   archivo de los
     *                                   jugadores
     * @throws ArchivoJugadoresException Se lanza esta excepción si el archivo no cumple con el
     *                                   formato esperado
     */
    public void modificarInformacionJugadores(File archivo)
            throws ArchivoJugadoresException, IOException {

        if (!archivo.exists())
            throw new IOException("HUEPUTAAAA ESCRIBA BIEN PERRO");
        else {

            BufferedReader br = new BufferedReader(new FileReader(archivo));

            String fuck;
            while ((fuck = br.readLine()) != null) {
                String equipo = null;

                if (fuck.startsWith("EQUIPO")) {
                    String[] arreglo = fuck.split(";");

                    equipo = arreglo[1];
                    String tecnico = arreglo[2];
                    String rutaImagen = arreglo[3];

                    try {
                        agregarEquipo(equipo, tecnico, rutaImagen); //Italia;Marcello Lippi;
                        // italia.png
                    ...
                    throw new ArchivoJugadoresException("FORMATO ILLEGAL");

                fuck = br.readLine();
                int numJugadores = Integer.parseInt(fuck);
                for (int i = 0; i < numJugadores; i++) {
                    fuck = br.readLine();

                    String[] infoJugador = fuck.split("-");

                    String nombre = infoJugador[0];
                    int edad = Integer.parseInt(infoJugador[1]);
                    String posicion = infoJugador[2];
                    double altura = Double.parseDouble(infoJugador[3]);
                    int peso = Integer.parseInt(infoJugador[4]);
                    int salario = Integer.parseInt(infoJugador[5]);
                    String rutaImagen = infoJugador[6];

                    try {
                        agregarJugadorAEquipo(equipo, nombre, edad, posicion, altura, peso,
                                                  salario,rutaImagen);
                    } catch (ElementoExisteException e) {
                        e.printStackTrace();
                    }
     ...
     * @throws ArchivoJugadoresException Se lanza esta excepción si el archivo no tiene el
     *                                   formato esperado
        ...
        if (nuevosDatos.length == 8) {
            ...
            if (equipo != null) {
                if (equipo.darJugador(nombreJugador) != null) {
                    ...
                    if (edad < 0 || posicion.equals("") || altura < 0 || peso < 0 || salario < 0
                            || imagen.equals("")) {
                        throw new ArchivoJugadoresException(
                                "La datos del jugador " + nombreJugador + " no son válidos");
                    }

                    equipo.modificarInformacionJugador(nombreJugador, edad, posicion, altura, peso,
                                                       salario, imagen);
                }
                else {
                    throw new ArchivoJugadoresException(
                            "El jugador con nombre " + nombreJugador + " no se encuentra en el "
                                    + "campeonato");
                }
            }
            else {
                throw new ArchivoJugadoresException(
                        "El archivo no tiene el formato esperado. El equipo con nombre "
                                + nombreEquipo + " no se encuentra en el campeonato");
            }
        }
        else {
            throw new ArchivoJugadoresException(
                    "El archivo no tiene el formato esperado. Se esperaban los ocho datos del "
                            + "jugador");
        }
    }
     ...
     *
     * @throws PersistenciaException Se lanza esta excepción si hay problemas guardando la
     *                               información del mundial en el archivo
     */
    ...
    /**
     * Registra en el archivo de log del programa toda la información referente a una excepción,
     ...
     *
     * @return Retorna true si hay un equipo que aparece repetido dentro de la lista de equipos.
